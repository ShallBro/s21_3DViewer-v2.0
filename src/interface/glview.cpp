...
void GlView::mouseMoveEvent(QMouseEvent *mouseEvent) {
  if (mouseEvent->buttons() == Qt::LeftButton) {
    ...
    controller.rotateOX(
        &vertex,
        K * static_cast<double>(mouseEvent->pos().y() - lastMousePosition.y()));

    controller.rotateOY(
        &vertex,
        K * static_cast<double>(lastMousePosition.x() - mouseEvent->pos().x()));
    update();
  } else if (mouseEvent->buttons() == Qt::MiddleButton) {
    updateStatusBarText(" [move] " + QString::number(mouseEvent->pos().x()) +
                        ":" + QString::number(mouseEvent->pos().y()));

    controller.moveOX(&vertex, 10 * K *
                                   static_cast<double>(mouseEvent->pos().x() -
                                                       lastMousePosition.x()));
    controller.moveOY(&vertex, 10 * K *
...
void GlView::wheelEvent(QWheelEvent *wheelEvent) {
  ...
  if (step > 0) {
    str = "In";
    controller.scale(&vertex, 1.1);
  } else {
    str = "Out";
    controller.scale(&vertex, 0.9);
  }
...
void GlView::open_file(char *filename) {
  if (!controller.ParseObject(filename, &vertex, &polygons, &polygons_size)) {
