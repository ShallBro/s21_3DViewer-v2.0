...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->statusBar->showMessage("Written by arnoldrh and usulelia.", 5000);
}

MainWindow::~MainWindow() {
  saveSettings();
  controller.kill_structs((openGLWidget->vertex), (openGLWidget->polygons),
...
void MainWindow::on_buttonUp_clicked() {
  if (openGLWidget->polygons_size != 0) {
    if (ui->radioButtonMove->isChecked()) {
      controller.moveOY(&(openGLWidget->vertex), K * openGLWidget->joy_step);
      openGLWidget->update();
    } else if (ui->radioButtonRotate->isChecked()) {
      controller.rotateOX(&(openGLWidget->vertex), -K * openGLWidget->joy_step);
      openGLWidget->update();
    } else if (ui->radioButtonZoom->isChecked()) {
      controller.scale(&(openGLWidget->vertex), 1 + K * openGLWidget->joy_step);
      openGLWidget->update();
    }
  }
}
...
void MainWindow::on_buttonLeft_clicked() {
  if (openGLWidget->polygons_size != 0) {
    if (ui->radioButtonMove->isChecked()) {
      controller.moveOX(&(openGLWidget->vertex), -K * openGLWidget->joy_step);
      openGLWidget->update();
    } else if (ui->radioButtonRotate->isChecked()) {
      controller.rotateOY(&(openGLWidget->vertex), K * openGLWidget->joy_step);
      openGLWidget->update();
    }
  }
}
...
void MainWindow::on_buttonZ1_clicked() {
  if (openGLWidget->polygons_size != 0) {
    if (ui->radioButtonMove->isChecked()) {
      controller.moveOZ(&(openGLWidget->vertex), K * openGLWidget->joy_step);
      openGLWidget->update();
    } else if (ui->radioButtonRotate->isChecked()) {
      controller.rotateOZ(&(openGLWidget->vertex), -K * openGLWidget->joy_step);
      openGLWidget->update();
    }
  }
}
